...
$(document).ready(function(){
  ...
  $('#product_tab_add_button').click(function(){
    index++;
    $('.widgets_tabs').append('<div class="widget-box product_tabs"><div class="widget-header header-color-blue2" style="padding: 5px 7px;"><h5><input type="text" class="input-xlarge title" placeholder="Tab Title"></h5><div class="widget-toolbar"><input type="hidden" class="id_for_collapse" value='+index+'><a href="#" onclick="collapse_tab('+index+'); return false;"style="color: #C7C5D1;"><i class="icon-chevron-up collapse" style="background-color: rgba(0,0,0,0);"></i></a> <a href="#" data-action="close" style="color: #F79B94;" ><i class="icon-remove"></i></a></div></div><div class="widget-body"><div class="widget-main"><textarea class="textarea_product_tabs" id="tab_desc'+index+'"></textarea></div></div></div>');
    initilize_editor();
  ...
  $('#add_product_form').ajaxForm({
    ...
    complete: function(xhr, response) {
      ...
      if (xhr.responseText !== undefined) {
        ...
        if (response_data.product_id !== undefined) {
          $("#product_id").val(response_data.product_id);
        }
      }
      ...
      if ($("#product_id").length > 0) {
        ...
        saveCustomFields(product_id, function(){
          save_product_option_items(product_id, function(xhr2){
            complete_add_product(xhr2);
          }, xhr);
        });
      }else{
        jQuery.post(CONFIG.get('URL')+'products/getLastID',{action:'getLastID'}, function(response2,status2){
          var product_id = JSON.parse(response2);
  ...
  $("#btn_select_brand").click(function(e){
    $("#modal-product-brand-selection").modal({
      backdrop : 'static',
      keyboard : false,
    });
  });
  $('#product-brand-table').dataTable({
    "bProcessing": true,
    "bServerSide": true,
    "sAjaxSource": CONFIG.get('URL')+"products/getProductBrands",
    "aoColumns": [
    ...
    "fnDrawCallback": function( oSettings ) {
      $(this).find(".btn-select-product-brand").each(function(e){
        $(this).click(function(){
          var selected_brand_id = $(this).attr('data-value');
          var selected_product_img = $("#product-brand-image-" + selected_brand_id).attr('src');
          var selected_product_label = $("#product-brand-label-" + selected_brand_id).html();
          
          $("#modal-product-brand-selection").modal('hide');
          $("#selected_brand_id").val(selected_brand_id);
          $("#selected_product_image").attr('src', selected_product_img);
          $("#selected_product_label").html(selected_product_label);
        });
      });
  ...
  $("#table-review").dataTable({
    "bProcessing": true,
    "bServerSide": true,
    "sAjaxSource": CONFIG.get('URL') + "products/review_processor/",
    "aoColumns": [
    {"bSortable": false},
    {"sWidth": "300px", },
    {"sWidth": "70px", "bSortable": false},
      ...
      $(".btn-review-edit").click(function(e){
        var id = $(this).attr('data-value');

        $("#product-review-manage-container").find('.accordion-toggle').removeClass('collapsed');
        $("#product-review-manage-container").find('.accordion-body').addClass('in').css('height','auto');

        var olc = cms_function.elements.overlay.clone();
        $("#product-review-manage-container").append(olc);


        $.post(CONFIG.get("URL") + "products/review_processor/",{
          action : 'get',
          review_id : id,
        },function(response){
          ...
          if (typeof response['status'] != 'undefined') {
            $("#review-id").val(response['data']['id']);
            $("#review-name").val(response['data']['name']);
            $("#review-email").val(response['data']['email']);
            $("#review-content").val(response['data']['content']);
            $("#review-rate").val(response['data']['rate']);
          }else{
            notification("Product Review", "Unrecognized Server Response", "gritter-error");
          }
          olc.remove();
          $("#review-name").focus();
        });
      ...
      aoData.push({"name": "action", "value": "table"});
      aoData.push({"name": "id", "value": $('#product_id').val()});
    }
  });

  $(".full-chosen").chosen({width:'100%'}).siblings('.chosen-container').find('.chosen-search').remove();
  $("select[name=status]").chosen({width:'100%'}).siblings('.chosen-container').find('.chosen-search').remove();
  $("#language").chosen({width:'100%'}).siblings('.chosen-container').find('.chosen-search').remove();
  $("#language").change(function(){
    get_product_detial(function(){
      proccess_slug($("#txt_url_slug").val());
      if (typeof setCustomField == 'function') {
        setCustomField();
      }
    });
  });
  ...
  $("#btn-delete-translation").click(function(e){
    ...
    bootbox.confirm("Are you sure you want to delete selected translation [<b>"+ selected_translation +"</b>]?", function(result){
      if (result) {
        jQuery.post(CONFIG.get('URL')+'products/delete_translation/',{
          id : $('#product_id').val(),
          lang : $("#language").val(),
        },
        function(response) {
  ...
  $("#product-review-add").click(function(e){
    if ($("#product_id").val() != "" && $("#review-name").val() != "" && $("#review-email").val() != "" && $("#review-rate").val()){
      bootbox.confirm("Proceed on adding Review?", function(result){
        if (result) {
          var data = {
            'id' : $("#review-id").val(),
            'guid' : $("#product_id").val(),
          ...
          $.post(CONFIG.get('URL') + "products/review_processor/",{
            ...
            if (cms_function.isJSON(response)) {
              ...
              if (typeof res['status'] != 'undefined') {
                notification("Product Review", res['message'], "gritter-success");
              }else{
                notification("Product Review", "Error: " + res['message'], "gritter-error");
              }
            }else{
              notification("Product Review", "Unrecognized response", "gritter-error");
            }

            $("#table-review").dataTable().fnDraw();
      ...
      notification("Product Review", "Please fill all the fields", "gritter-error");
    }
  });

  $("#product-review-clear").click(function(e){
    $("#review-id").val("")
    $("#review-name").val("")
    $("#review-email").val("")
    $("#review-content").val("")
  ...
  $("#toggle-main-page-content").change(function(e){
    var v = $(this).is(':checked') ? 'shown' : 'hidden';
    cookie_set_content('hide-main-content', v=='shown');
    check_page_content();
  });
  $("#toggle-other-settings").change(function(e){
    var v = $(this).is(':checked') ? 'shown' : 'hidden';
    cookie_set_content('hide-settings', v=='shown');
    check_setting_content();
  });
  $("#toggle-page-layout").change(function(e){
    var v = $(this).is(':checked') ? 'shown' : 'hidden';
    cookie_set_content('hide-page-layout', v=='shown');
...
function check_page_content(){
  if (cookie_check_content('hide-main-content') != 'false') {
    $(".main-page-content").closest('.widget-body').show();
    $("#toggle-main-page-content").prop('checked', 'checked');
  }else{
    $(".main-page-content").closest('.widget-body').hide();
    $("#toggle-main-page-content").removeProp('checked');
  }
}
function check_setting_content(){
  if (cookie_check_content('hide-settings') != 'false') {
    $("#product-main-settings").show();
    $("#toggle-other-settings").prop('checked', 'checked');
  }else{
    $("#product-main-settings").hide();
    $("#toggle-other-settings").removeProp('checked');
  }
}
function check_options_content(){
  if (cookie_check_content('hide-page-layout') != 'false') {
    $("#product-page-options").show();
    $("#toggle-page-layout").prop('checked', 'checked');
  }else{
    $("#product-page-options").hide();
    $("#toggle-page-layout").removeProp('checked');
  }
}

function cookie_set_content(name, shown_flag){
  var additional_day = 1 * 86400; // default day count
  var base_time = 1; // 86400 = one day

  var d = new Date();
  d.setDate(d.getDate())
  d.setSeconds(d.getSeconds() + ( base_time * additional_day));

  document.cookie = name + " = "+ (shown_flag ? "true" : "false") +"; expires = " + d.toUTCString() + "; path=/;";
}

function cookie_check_content(name){
...
function set_product_option_values(the_tag){
  ...
  function populate_option_item_list(set_options, set_options_values){
    ...
    var default_product_sku = $("#txt_sku").val() != "" ? $("#txt_sku").val() : "";
  ...
  function load_product_desc(){
    tinyMCE.init({
      ...
      height : 300,
      toolbar:[
        "nonbreaking forecolor backcolor undo redo styleselect bold italic link image imageuploader cmsmedia",
      ...
      init_instance_callback : function (){
        get_product_detial(function(){
          var slug = convertToSlug($("#txt_url_slug").val());
          var id = $("#product_id").length > 0 ? $("#product_id").val() : 0;
          var lang = $("#language").length ? $("#language").val() : '';
          $.post(CONFIG.get('URL')+'products/get_available_slug/', {action: 'get', slug:slug, lang:lang, id:id},function(response, status){
            var result = JSON.parse(response);
            if (typeof result['permalink'] != 'undefined') {
  ...
  function initilize_editor(){
    tinyMCE.init({
      selector:'.textarea_product_tabs',
      menubar: " view edit format table tools",
      height : 300,
      toolbar:[
        "nonbreaking forecolor backcolor undo redo styleselect bold italic link image imageuploader cmsmedia",
...
function addData(){
  ...
  $('.product_tabs').each(function(index){
    var object_tab = {};
    var id = $.trim($(this).find('.tab-id').val());
...
function validateForm(){
  ...
  if(arr.length  == 0)
  {
    ...
    notification("Product", "Please Select Category.", "gritter-error");
    error = true;
  }
  else
    jQuery('div').remove('#errorCategory');
...
function updateTreeview(){
  // $( "#content" ).load( CONFIG.get('URL')+'products/getData/', function( response, status, xhr ) {
    $.post( CONFIG.get('URL')+'products/getData/', function( response, status, xhr ) {
      if ( status == "success" ) {
        $('#tags').empty();
  ...
  function get_product_detial(fn){
    ...
    function(response,status){
      ...
      if (typeof response['detail']['product_name'] != 'undefined') {
        $("#txt_product_name").val(response['detail']['product_name']);
      }
      if (typeof response['detail']['seo_title'] != 'undefined') {
        $("#seo_title").val(response['detail']['seo_title']);
      }
      if (typeof response['detail']['seo_description'] != 'undefined') {
        $("#seo_description").val(response['detail']['seo_description']);
      }
      if (typeof response['tabs'] != 'undefined') {
        $.each(response['tabs'], function(k, v){
          if($("#widget_" + v['id']).length > 0){
            $("#widget_" + v['id']).find('.title').val(v['tab_title']);
            var tab_id = $("#widget_" + v['id']).find('.textarea_product_tabs').attr("id");
            tinyMCE.get(tab_id).setContent(v['tab_content']);
          }
        });
      }
      if (typeof response['detail']['url_slug'] != 'undefined') {
        $("#txt_url_slug").val(response['detail']['url_slug']);
      }
